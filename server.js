...
async function getSID(uid) {
  try {
    ...
    return res.data?.data?.sid || res.data?.data?.user_sid || 'N/A';
  } catch (e) {
    return 'N/A';
  }
}
...
app.get('/api/live-status', async (req, res) => {
  try {
    ...
    const [refreshRes, resultRes] = await Promise.all([
      axios.get(`https://m.starmakerstudios.com/go-v1/rhapsody-music/game-refresh?_sx_ts=${ts}`, { headers }),
      axios.get(`https://m.starmakerstudios.com/go-v1/rhapsody-music/game-result?_sx_ts=${ts}`, { headers })
    ]);
    ...
    for (let i = 0; i < Math.min(rawList.length, 3); i++) {
      ...
      if (!uid && (u.user_cover || u.profile_image)) {
        const m = (u.user_cover || u.profile_image).match(/users\/(\d+)\//);
        uid = m ? m[1] : null;
      }
      ...
      if (uid) {
        ...
        top3.push({
          rank: i + 1,
          sid: sid,
          name: u.name || u.stage_name || 'Player',
    ...
    res.json({
      success: true,
      round: roundData.curr_round || roundData.round || 'N/A',
